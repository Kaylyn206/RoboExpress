...
int servo4Position = 90; // Initial position of servo4 (90°)
    ...
    if (&servoA == &servo3) { // Special handling for servo3
      switch (command) {
        ...
        case 'L': /* servoB.write(0); break;   // Move left */
          servo4Position -= 5; // Decrement position gradually
        if (servo4Position < 0) servo4Position = 0;
        servoB.write(servo4Position);
        break;
        case 'R': /* servoB.write(180); break; // Move right */
          servo4Position += 5; // Increment position gradually
        if (servo4Position > 180) servo4Position = 180;
        servoB.write(servo4Position);
        break;
        default:
